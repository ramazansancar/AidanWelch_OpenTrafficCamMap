...
class Karacabey:
    ...
    async def kameralar(self) -> dict[str, str]:
        istek = await self.oturum.get("https://wowza.yayin.com.tr/playlist/karacabey/playlist_karacabey.json")
        return istek.json()
    ...
    async def getir(self) -> dict[list[dict]]:
        ...
        for kamera_veri in kameralar.get("playlist", []):
            latitude, longitude = await str2latlng(f"{kamera_veri['title']}, Karacabey, Bursa, Türkiye")

            veri["Karacabey"].append({
...
async def basla():
    ...
    konsol.log(f"[yellow][Bursa][Karacabey] [+] {len(gelen_veriler['Karacabey'])} Adet Kamera Bulundu")
    ...
    if gelen_veriler == mevcut_veriler.get("Bursa"):
        konsol.log("[red][Bursa][Karacabey] [!] Yeni Veri Yok")
        return

    if mevcut_veriler.get("Bursa"):
        del mevcut_veriler["Bursa"]
    mevcut_veriler["Bursa"] = gelen_veriler
    ...
    konsol.log(f"[green][Bursa][Karacabey] [+] {len(gelen_veriler['Karacabey'])} Adet Kamera Eklendi")